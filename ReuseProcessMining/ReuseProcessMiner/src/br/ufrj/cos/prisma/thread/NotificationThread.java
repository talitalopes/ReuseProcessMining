 ...
 * where all work should be performed. Add any listener to update upon
 * completion, then create a new thread with this new object and run.
 * 
 * @author Greg Cope
 * 
 */
...
public abstract class NotificationThread implements Runnable {
	...
	private java.util.List<TaskListener> listeners = Collections
			.synchronizedList(new ArrayList<TaskListener>());
	...
	private final void notifyListeners() {
		synchronized (listeners) {
			for (TaskListener listener : listeners) {
				listener.threadComplete(this, projects);
			}
		}
	}
